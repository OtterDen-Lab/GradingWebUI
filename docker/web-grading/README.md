...
- `make publish v0.8.1` (publish `ghcr.io/otterden-lab/gradingwebui:v0.8.1` and `latest` for `linux/amd64`)
...
- Port 8765 already in use → Stop other services or change port in `docker-compose.yml`

### Can't access at localhost:8765
...
- Try accessing from `http://127.0.0.1:8765` instead
...
```yaml
ports:
  - "8080:8765"  # Change first number only
```
...
### Documentation
- Main documentation: `../../WebUI/docs/`
- User guide: `../../WebUI/docs/user-guide.md`
- Architecture: `../../WebUI/docs/architecture.md`
...
See `../../WebUI/planning/deployment-roadmap.md` for production deployment guide.
...
cd /path/to/GradingWebUI
python -m uvicorn grading_web_ui.web_api.main:app --reload --port 8765
```
...
services:
  web:
    volumes:
      - ../../src:/app/src
    command: python -m uvicorn grading_web_ui.web_api.main:app --reload --host 0.0.0.0 --port 8765
```
